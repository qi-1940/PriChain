...
4. **t1 需要 t3 占用的资源，被阻塞**
   ...
   t1 运行过程中需要访问 t3 占用的资源（比如同一个互斥锁），但资源被 t3 占用，t1 被阻塞，等待资源释放
...
​			统一框架：分析并抽象mutex、rwmutex、rwsem共性，构建统一的优先级继承与链式传递框架，支持多层次锁依赖关系
...
- 集成优先级继承机制，实现锁持有者优先级的动态提升与递归传播，确保高优先级任务及时获得资源
